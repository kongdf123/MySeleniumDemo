...
namespace OrderDemo.Controllers
{
    ...
    {
        private IOrderBaseDb db;

		public ProductsController() {
			db = new OrderBaseDb();
		}

		public ProductsController(IOrderBaseDb db) {
			this.db = db;
		}
        ...
        {
			var model= db.Query<Product>().Take(5).ToList();
            return View(model);
        }
        ...
        public ActionResult Details(int? id)
        {
            ...
            Product product = db.Query<Product>().FirstOrDefault(x=>x.Id==id);
            if (product == null)
            {
                return HttpNotFound();
            }
